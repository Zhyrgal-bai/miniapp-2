...
export function CoffeePdpContent({
  product,
  businessId,
  businessType,
  ...
  const heroFacts = useMemo(() => {
    const temp = pickString(attrs, "hotOrCold");
    return [
      px.sizes.length > 0 ? `${px.sizes.length} размеров` : null,
      temp ? (HOT_OR_COLD_RU[temp] ?? temp) : null,
      ...
      <div className="px-layout">
        ...
        <div className="px-main">
          <header className="px-head">
            ...
            <p className="coffee-pdp__notice">
              Настройте молоко, сахар и сиропы перед добавлением в заказ.
            </p>
          </header>
          ...
          {!px.outOfStock ? (
            <>
              ...
              <div className="coffee-pdp__options">
